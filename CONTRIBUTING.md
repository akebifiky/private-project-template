...
- 作業内容や開発対象、またはチケット管理システム上のチケット ID などをもとに、 `topic/[branch-name]` と命名されるものとします。 [RECOMMENDED]
  - 例: `topic/user-management`, `topic/ticket-123`
...
- ブランチのマージは、原則として GitLab 上の [マージリクエスト機能](https://gitlab-docs.creationline.com/ee/user/project/merge_requests/creating_merge_requests.html) を介して実行されるものとします。 [MUST]
  - この際、コードオーナー (後述の [CODEOWNERS の設定](#コードオーナーの設定) を参照) による承認が必要です。
...
- 各リリースバージョンは、 Backlog におけるマイルストーンと一致しているものします [RECOMMENDED]
...
- コミットに対して Backlog チケットが紐づいている場合、メッセージの冒頭にチケット番号を記載してください。 [RECOMMENDED]
...
```plaintext
:sparkles: [SOME_PROJECT-123] 新規コンポーネント機能 Sample を追加
```
...
```plaintext
:bug: [SOME_PROJECT-123] コンポーネント機能 Sample においてタイトルが正しく表示されない問題に対応
