...
import AdrIndexList from "@site/src/components/AdrIndexList";
...
- 参照する PR/Issue/他ドキュメントは、本文 (Context・Consequences など) 中の該当箇所で直接リンク形式で記載する
- 本文中に登場しない関連資料がある場合は、末尾に `## 関連リンク` セクションを設けて記載する
- 後述の「Index」一覧は、`docs/dev-docs` のビルド時 (Docusaurus プラグイン `src/plugins/adr-tc-index` の `loadContent`/`contentLoaded`) に、ADR ディレクトリ配下の各ファイルの Frontmatter・見出しから動的に構築される (ADR-0159) 。ADR ファイルを追加・変更するだけで一覧に反映されるため、一覧セクション自体を手動で編集する必要はない
- AI エージェントがビルドを介さずファイルとして直接 ADR を参照する場合は、`npm run search:adr` (`docs/dev-docs/scripts/search-adr.ts`) を実行し、その標準出力を参照する (検索キーワードを絞り込みたい場合は `npm run search:adr -- --query=<キーワード>`)
...
<!-- 本文中に登場しない関連資料がある場合のみ、以下のセクションを追加する
